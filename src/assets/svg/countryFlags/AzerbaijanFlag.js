import * as React from "react"
import Svg, { G, Path, Defs, ClipPath } from "react-native-svg"
const SvgComponent = (props) => (
  <Svg
    xmlns="http://www.w3.org/2000/svg"
    width={512}
    height={512}
    fill="none"
    {...props}
  >
    <G clipPath="url(#a)">
      <Path
        fill="#D80027"
        d="M512 256c0-31.314-5.632-61.311-15.923-89.043L256 155.826l-240.077 11.13C5.633 194.689 0 224.686 0 256s5.633 61.311 15.923 89.043L256 356.174l240.077-11.13C506.368 317.311 512 287.314 512 256Z"
      />
      <Path
        fill="#6DA544"
        d="M256 512c110.071 0 203.906-69.472 240.077-166.957H15.923C52.093 442.528 145.929 512 256 512Z"
      />
      <Path
        fill="#338AF3"
        d="M15.923 166.957h480.155C459.906 69.472 366.071 0 256 0S52.094 69.472 15.923 166.957Z"
      />
      <Path
        fill="#F0F0F0"
        d="M261.565 328.348c-39.956 0-72.348-32.392-72.348-72.348 0-39.956 32.392-72.348 72.348-72.348 12.458 0 24.181 3.15 34.415 8.696-16.056-15.701-38.012-25.392-62.241-25.392-49.178 0-89.043 39.866-89.043 89.043 0 49.177 39.866 89.043 89.043 89.043 24.23 0 46.186-9.691 62.241-25.392-10.234 5.548-21.957 8.698-34.415 8.698Z"
      />
      <Path
        fill="#F0F0F0"
        d="m317.217 205.913 9.583 26.951 25.834-12.282-12.281 25.834L367.304 256l-26.951 9.584 12.281 25.834-25.834-12.282-9.583 26.951-9.582-26.951-25.835 12.282 12.282-25.834L267.13 256l26.952-9.584-12.282-25.834 25.835 12.282 9.582-26.951Z"
      />
    </G>
    <Defs>
      <ClipPath id="a">
        <Path fill="#fff" d="M0 0h512v512H0z" />
      </ClipPath>
    </Defs>
  </Svg>
)
export default SvgComponent
